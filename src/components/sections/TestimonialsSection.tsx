          ...
          {isGoogleReviews && businessData.rating && (
            ...
            >
              <div className="flex gap-0.5">
                {[...Array(5)].map((_, i) => (
                  <Star key={i} size={15} className={i < Math.round(parseFloat(businessData.rating)) ? "text-yellow-500 fill-yellow-500" : "text-foreground/15"} />
                ))}
              </div>
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-5 md:gap-6">
          {testimonials.map((testimonial: any, i: number) => (
            <motion.div
              ...
              className="relative p-6 md:p-7 rounded-2xl bg-background border border-border hover:border-primary/15 transition-all duration-300 group"
            >
              {/* Quote decoration */}
              <div className="absolute top-5 right-5 opacity-[0.06] group-hover:opacity-[0.1] transition-opacity">
              ...
              <div className="flex items-center gap-3 pt-4 border-t border-border/50">
                <div className="w-9 h-9 rounded-full bg-gradient-to-br from-primary/80 to-accent/80 flex items-center justify-center shrink-0">
